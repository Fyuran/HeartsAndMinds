...
/* ----------------------------------------------------------------------------
Function: btc_int_fnc_setCaptives
...
Examples:
    (begin example)
        [cursorObject] spawn btc_int_fnc_setCaptives;
    (end)
...
if(!canSuspend) exitWith {
  ["must be called with spawn", __FILE__, [false, true, true], true] call btc_debug_fnc_message;  
};
...
[_remainEnemyUnits, {
    _this apply {
        _action = ["JailCaptive", localize "STR_BTC_HAM_ACTION_DETAIN", "core\img\jail_captive.paa", {
            _jail = btc_jails select (btc_jails findIf {(_target distanceSqr _x) <= DISTSQR });

            [_target, _player, _jail] remoteExecCall ["btc_info_fnc_jail", [0, 2] select isMultiplayer];
            [_target, 0, ["ACE_MainActions","JailCaptive"]] remoteExecCall ["ace_interact_menu_fnc_removeActionFromObject", [0, -2] select isMultiplayer];
        }, {
...
_remainEnemyUnits apply {
    ...
    if(_currentWeapon isNotEqualTo "") then {
        ...
        _weaponHolder setPos position _weaponHolder; ////required to sync dir https://community.bistudio.com/wiki/setDir#Notes
    };
