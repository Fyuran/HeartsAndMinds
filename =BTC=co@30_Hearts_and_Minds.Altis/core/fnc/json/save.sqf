...
[26] remoteExecCall ["btc_fnc_show_hint", 0];
...
private _cities_status = createHashMap;
{
    ...
    if (_name isEqualTo "") then {_name = _y getVariable "id"};
    ...
    private _hash = ["id", "initialized", "spawn_more", "occupied",  
        ...
        _y getVariable "id", 
        _y getVariable "initialized", 
        _y getVariable "spawn_more", 
        _y getVariable "occupied",
...
private _array_ho = createHashMap;
{
    ...
    private _hash = [
    "pos", "id", "rinf_time", 
        "cap_time", "assigned_to", "markers"] createHashMapFromArray [
            [_xx, _yy, 0],  
            _x getVariable ["id", 0],
    ...
    _array_ho set [_forEachIndex, _hash];
...
_array_cache set ["cache", [
    "pos", "cache_n", "cache_info",
        "markers", "cache_pictures", "isChem", "cache_unitsSpawned"
] createHashMapFromArray [
...
private _fobs = createHashMap;
{
    if !(isNull ((btc_fobs select 2) select _forEachIndex)) then {
        ...
        _fobs set [_forEachIndex, [markerText _x, _pos, _direction]];
    };
...
private _vehiclesInCargo = _vehicles - _vehiclesNotInCargo;
{
    ...
    private _hash = [
        "type", "pos", "dir", "fuel", "allHitPointsDamage", "cargo",
            "inventory", "customization",  "btc_EDENinventory", "vectorPos", "isContaminated", 
                "flagTexture", "turretMagazines",
                    "tagTexture", "properties" //Vehicle in Vehicle
    ]createHashMapFromArray[
        _type, _pos, _dir, fuel _x, getAllHitPointsDamage _x, 
        _cargo, _inventory, [_x] call btc_veh_fnc_propertiesGet, 
        _x getVariable ["btc_EDENinventory", []], _vectorPos, _isContaminated,
...
private _array_obj = createHashMap;
{
    if !(!alive _x || isNull _x) then {
        ...
        private _hash = 
        ["typeOf", "pos", "direction", "ace_cargo_loaded",
            "btc_log_fnc_inventoryGet", "vectorDirUp", "isChem", "dogtags",
                "flagTexture", "magazinesAllTurrets", "ace_cargo_customName", "btc_tag_vehicle",
                    "properties"
...
{
    private _hash = 
    ["markerText", "markerPos", "markerColor", "markerType",  
...
[_returnString, 1, [0,1,0,1]] call CBA_fnc_notify;