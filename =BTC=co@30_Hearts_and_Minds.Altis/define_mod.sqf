...
private _playerUnitsFactions = []; //use units with gear similiar to players
allPlayers apply {_playerUnitsFactions pushBackUnique getText(configfile >> "CfgVehicles" >> typeOf _x >> "faction")};
btc_friendly_type_units = ([_playerUnitsFactions] call btc_mil_fnc_class) select 1;
