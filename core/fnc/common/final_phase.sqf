...
[["btc_sze", "btc_m"], 2] call FUNC(task,create);
...
btc_city_remaining = [];
{
    if (_y getVariable ["type", ""] != "NameMarine") then {
        ...
        private _marker = createMarkerLocal [format ["city_%1", position _y], position _y];
        _marker setMarkerShapeLocal "ELLIPSE";
        _marker setMarkerBrushLocal "SolidBorder";
        _marker setMarkerSizeLocal [_cachingRadius, _cachingRadius];
...
["btc_m", "SUCCEEDED"] call FUNC(task,setState);
