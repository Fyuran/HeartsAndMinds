...
if(_isBoat) exitWith {
    ...
    [["%1: P:%2 is on boat, aborting", __FILE_NAME__, _group getVariable ["btc_patrol_id", 0]], 2, "patrol"] call FUNC(debug,message);
    #endif
};
...
if(_fobs isEqualTo []) exitWith {
    #ifdef BTC_DEBUG_PATROL
    [["%1: P:%2 _fobs is empty, aborting", __FILE_NAME__, _group getVariable ["btc_patrol_id", 0]], 2, "patrol"] call FUNC(debug,message);
    #endif
};
#ifdef BTC_DEBUG_PATROL
[["%1: P:%2 FOB WP checking", __FILE_NAME__, _group getVariable ["btc_patrol_id", 0]], 2, "patrol"] call FUNC(debug,message);
#endif
...
if(_fobIndex isNotEqualTo -1) then {
    ...
    if(!alive _fob) exitWith {
        #ifdef BTC_DEBUG_PATROL
        [["%1: P:%1 FOB is Null/dead", __FILE_NAME__, _group getVariable ["btc_patrol_id", 0]], 6, "patrol"] call FUNC(debug,message);
        #endif
    };

    [_group, _fob, 100, "PATROL"] call FUNC(mil,addWP);

    [{
        (_this select 0) inArea (((_this select 1) getVariable["FOB_Triggers", []]) select 0)
    }, {
        [_EVENT_FOB_ATTACK_, (_this select 1)] call FUNC(event,eventManager);
    ...
    if (!isNil {_group getVariable "btc_patrol_id"}) then {
        ...
        [["%1: P:%2 Found fob, redirecting patrol", __FILE_NAME__, _patrol_id], 2, "patrol"] call FUNC(debug,message);
    };
    #endif
};