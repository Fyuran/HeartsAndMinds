...
private _array = [_tower] call FUNC(log,get_nottowable);

if ((_array findIf {_towed isKindOf _x}) != -1) exitWith {
    private _string_array = "";
    {
        _string_array = _string_array + ", " + _x;
    } forEach (([_tower] call FUNC(log,get_nottowable)) - ["Truck_F"]);
...
private _model_front_towed = ([_towed] call FUNC(tow,hitch_points)) select 0;
