...
if (btc_p_db_autoRestartTime > 0) then {
    [{
        ...
        [FUNC(db,autoRestart), [], btc_db_warningTimeAutoRestart * 60] call CBA_fnc_waitAndExecute;
...
if (btc_p_db_autoRestartHour isNotEqualTo []) then {
    [{
        [{systemTime select 3 in btc_p_db_autoRestartHour}, FUNC(db,autoRestart)] call CBA_fnc_waitUntilAndExecute;
