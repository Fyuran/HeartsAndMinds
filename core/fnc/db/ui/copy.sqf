...
[["%1: Copying JSON file to %2", __FILE_NAME__, _path], 2, "json/ui"] call FUNC(debug,message);
#endif
private _returnString = ("btc_ArmaToJSON" callExtension ["copyFile", [_path]]) select 0;

if(_custom_hint isEqualTo "") then {
	[[_returnString, 1, [0, 0.5, 0.5, 1]]] call FUNC(common,show_custom_hint);
} else {
	[[_custom_hint, 1, [0, 0.5, 0.5, 1]]] call FUNC(common,show_custom_hint);
};
